...
const SelectTicket = () => {
    ...
    useEffect(() => {
      axios.get(`${protocol}://${ip_address}:${port_address}/api/ticket_entry`)
            .then(res => {
                setData(res.data);
    ...
    const columns = [
        ...
        {
            name: 'Options',
            cell: row => <Link to={`/pnlat/manage-ticket/${row.id}`}>more..</Link>,
        },
    ];
  ...
  return (
    ...
    <Link onClick={() => navigate(-1)}  class="btn btn-succes mt-2 pb-1 new-account-ticket float-ends">
        <FaAngleLeft size={25} /> 
            ...
            <Link to='/pnlat/balance' className='btn mt-2 btn-success new-account-ticke float-end'><FaPlus  size={18} /> Create </Link>
    <br />
          <br />
          {/* <!-- DataTales Example --> */}
                <div class="car shado mb-4 ">
                        ...
                        <div class="card-bod ">
                            ...
                            <h6 className="text-muted mt-"> Ticket(s) showing: <span className='font-weight-bold text-primary' >
